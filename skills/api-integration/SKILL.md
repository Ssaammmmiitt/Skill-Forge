...
- Vite proxy: `/api` → backend URL (default `http://localhost:5000`, configured in `vite.config.js`)
- Backend: FastAPI on port 5000 (`python -m api.app`)
...
- [ ] If the FastAPI server is off, the page shows an error message (not a crash)
